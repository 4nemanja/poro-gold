...
import type { GiftConfig } from "@/lib/data";

export function GiftConfigModal({ config }: { config: GiftConfig }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md bg-white rounded-xl shadow-xl border border-zinc-200">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200">
              <h3 className="text-sm font-semibold text-zinc-900">Edit Gift System</h3>
              <button onClick={() => setOpen(false)} className="text-zinc-400 hover:text-zinc-700"><X size={18} /></button>
            </div>
            <form onSubmit={submit} className="p-5 space-y-4">
              <label className="flex flex-col gap-1 text-xs font-medium text-zinc-500">
                Invested (USD)
                <input name="invested_usd" type="number" step="0.01" min="0" defaultValue={config.invested_usd} className={cls} required />
              </label>
              <label className="flex flex-col gap-1 text-xs font-medium text-zinc-500">
                V-Bucks Stock (total bought)
                <input name="vbucks_stock" type="number" step="1" min="0" defaultValue={config.vbucks_stock} className={cls} required />
              </label>
              <label className="flex flex-col gap-1 text-xs font-medium text-zinc-500">
                Note
                <input name="note" defaultValue={config.note} className={cls} />
              </label>
